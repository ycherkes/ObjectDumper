...
using System.IO;
using ObjectFormatter.CodeDom.Embedded.ms.Resources;

// We keep this class in Common to allow utilizing it without taking a dependency on System.CodeDom
#if CODEDOM
namespace ObjectFormatter.CodeDom.Embedded.ms.Common.src.Sys.IO
#else
...
#endif
{
    ...
    class TempFileCollection : ICollection, IDisposable
    {
        ...
        public string AddExtension(string fileExtension, bool keepFile)
        {
            ...
            {
                throw new ArgumentException(SR.Format("Invalid argument: {0}", nameof(fileExtension)), nameof(fileExtension));
            }
        ...
        public void AddFile(string fileName, bool keepFile)
        {
            ...
            {
                throw new ArgumentException(SR.Format("Invalid null empty argument: {0}", nameof(fileName)), nameof(fileName));
            }

            if (_files[fileName] != null)
            {
                throw new ArgumentException(SR.Format("Duplicate File Name: {0}", fileName), nameof(fileName));
            }
